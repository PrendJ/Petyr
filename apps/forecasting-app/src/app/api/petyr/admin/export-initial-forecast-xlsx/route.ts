...
import { buildInitialForecastWorkbookXlsx } from "@/services/petyrInitialForecastExcelService";
...
export async function GET(request: Request) {
  ...
  try {
    const workbook = await buildInitialForecastWorkbookXlsx({ year });
    const filename = `petyr-initial-forecast-${year}.xlsx`;
    ...
    return NextResponse.json(
      {
        error: "Unable to export Initial Forecast Excel workbook",
